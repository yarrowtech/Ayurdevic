export function validateProduct(body) {
  const { name, category, price, offerPrice, image, description, inStock } = body;
  ...
  return { name: name.trim(), category: category.trim(), price, offerPrice, image, description, inStock };
}
