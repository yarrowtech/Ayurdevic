...
import { getProductPrice } from "../configs/productPrice.js";
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    for (const { productId, quantity } of data.items) {
      ...
      items.push({ product: product._id, name: product.name, image: product.image?.[0] || "", price: getProductPrice(product), quantity });
    }
